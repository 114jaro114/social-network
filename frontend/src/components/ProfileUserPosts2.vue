...
<v-container class="center p-0">
    ...
    <v-card class="w-100 rounded" elevation="0">
      <v-flex xs12 sm12>
        <v-layout wrap column>
          <v-flex xs12>
            <v-sheet elevation="0" class="py-4 px-1 rounded d-flex justify-center" v-if="$root.profilePosts.getAllPosts.loadingPosts == true">
              <v-chip-group>
  ...
  <v-row class="ml-0 mr-0 mb-3" v-if="$root.profilePosts.getAllPosts.loadingPosts != true">
    <v-card class="w-100 rounded" elevation="0">
                ...
                <v-tooltip bottom v-if="isThereMyShare(post_data.postsharemodel)">
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn v-bind="attrs" v-on="on" text rounded outlined color="primary" @click="removeShare(post_data.id)">
                      <v-icon class="mr-1 primary--text" small>mdi-share</v-icon><span class="font-weight-bold">Zdieľané</span>
                    </v-btn>
                  </template>
        ...
        <v-container style="display:none" :id="`showPostDetails${post_data.id}`" class="p-0">
          ...
          <v-list dense>
            <template v-for="(post_comment_data, index) in post_data.postcommentmodel">
              <v-container class="p-0" append-icon="" v-if="post_data.id == post_comment_data.post_id" :key="'comments'+index">
                <v-list-item class="w-100 ">
                  ...
                  <v-list-item-content>
                    <v-card class="p-2 rounded-lg card-comments" elevation="0">
                      <v-list-item-title class="d-flex justify-start text-subtitle-2 font-weight-bold p-0 pl-1">
                        <v-menu top offset-y transition="scale-transition" :close-on-content-click="false">
                          <template v-slot:activator="{ on, attrs }">
                            <v-chip @click="mouseOver(post_comment_data.userpostcommentmodel.id)" v-bind="attrs" v-on="on" small>
                      ...
                      <v-list-item-subtitle class="text-left primary--text font-weight-bold p-0 pl-1 pt-1" v-if="post_comment_data.chip == null">{{post_comment_data.text}}</v-list-item-subtitle>
                      <v-list-item-subtitle class="text-left primary--text font-weight-bold p-0 pl-1 pt-1" v-else>
                      ...
                      <div class="position-absolute mr-3" style="right:0;bottom:0">
                        <!-- ///////////////////////////////////////////////////////////////////////// -->
                        <v-dialog max-width="600" :retain-focus="false" v-if="post_comment_data.count_reactions != 0">
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn small text class="hidden-sm-and-down primary--color rounded-pill" v-bind="attrs" v-on="on">
                              <div class="">
                            ...
                            <v-btn x-small text class="hidden-md-and-up primary--color rounded-pill" :min-width="40" v-bind="attrs" v-on="on">
                              <div class="">
                                <v-menu open-on-hover bottom offset-y v-if="post_comment_data.like != 0">
                                  <template v-slot:activator="{ on, attrs }">
                      ...
                      <v-menu open-on-hover top offset-y min-width="200px" rounded="pill">
                        <template v-slot:activator="{ on }">
                          <v-btn class="mt-1" text rounded x-small outlined v-on="on" color="primary" v-if="isThereMyReaction(post_comment_data.commentreactionmodel2)" @click="removeRateComment(post_comment_data.id, 'posts_comments')">
                            ...
                            <span style="color:#ffb71d" v-if="isThereMyReactionTypeReaction(post_comment_data.commentreactionmodel2)['rposteaction_type'] == 'funny'">
                              <v-avatar class="mr-1" size="14">
                                  <v-img :lazy-src="require('../assets/img/emoticons/face_open_mouth_smiling_eyes.png')" :src="require('../assets/img/emoticons/face_open_mouth_smiling_eyes.png')" />
                              </v-avatar>
                    ...
                    <v-row class="ml-0 mr-0 mt-0 pt-1 pb-1">
                      <v-chip @click="show_comments(post_comment_data.id)" v-if="post_comment_data.count_comments == 1" outlined small class="d-flex justify-start caption primary--text font-weight-bold"
                      ...
                      <v-chip @click="show_comments(post_comment_data.id)" v-if="post_comment_data.count_comments > 1" outlined small class="d-flex justify-start caption primary--text font-weight-bold"
                        ...
                        <v-icon left class="ml-2" style="display:none" v-bind:class="'btn_answers_on_comments_arrow_down' + post_comment_data.id" small>mdi-chevron-down</v-icon>
                        <v-icon left class="ml-2" style="display:block" v-bind:class="'btn_answers_on_comments_arrow_up' + post_comment_data.id" small>mdi-chevron-up</v-icon>
                      </v-chip>
                    </v-row>
                  </v-list-item-content>
                </v-list-item>
                      ...
                      <v-list-item-content>
                        <v-card class="p-2 rounded-lg card-comments" elevation="0" color="#f5f5f5">
                          <v-list-item-title class="d-flex justify-start text-subtitle-2 font-weight-bold p-0 pl-1">
                            <v-menu top offset-y transition="scale-transition" :close-on-content-click="false">
                              <template v-slot:activator="{ on, attrs }">
                                <v-chip @click="mouseOver(answer_on_comment_data.useransweroncommentmodel.id)" v-bind="attrs" v-on="on" small>
                          ...
                          <v-list-item-subtitle class="text-left primary--text font-weight-bold p-0 pl-1 pt-1" v-if="answer_on_comment_data.chip == null">{{answer_on_comment_data.text}}</v-list-item-subtitle>
                          <v-list-item-subtitle class="text-left primary--text font-weight-bold p-0 pl-1 pt-1" v-else>
  ...
  export default {
    ...
    methods: {
      ...
      focusInputAnswerOnComment(item) {
        for (var i = 1; i < this.chipTextAnswerOnComment.length; i++) {
          if (document.getElementById(`itemsPostComment${i}`).style != null) {
            document.getElementById(`itemsPostComment${i}`).style.display = 'none';
          }
        }
      ...
      comment(index) {
        ...
        axios.post(api, {
              ...
              chip: this.chipTextPostComment[index]
            },
            config
          )
          .then(() => {
            // this.$root.profilePosts.getAllPosts.allPosts.forEach((elem, index2) => {
            //   if (elem.id == index) {
            //     this.$root.profilePosts.getAllPosts.allPosts[index2] = res.data[0];
            //   }
            // })
      ...
      answerOnComment(index) {
        ...
        axios.post(api, {
              ...
              chip: this.chipTextAnswerOnComment[index]
            },
            config
          )
          .then((res) => {
