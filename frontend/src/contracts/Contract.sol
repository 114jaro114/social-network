...
contract Contract{
    ...
    mapping(bytes32 => User) public me;
    ...
    // Event which will notify update userinfo.
    event UpdateUserInfo();

    // Event which will notify new user.
    event setUserInfo();
    ...
    function getMe(bytes32 _id) public view returns (bytes32, bytes32, bytes32, bytes32){
        for(uint i = 0; i < users.length; i++) {
            if(users[i].id == _id) {
                return (users[i].id, users[i].name, users[i].email, users[i].avatar);
            }
        }
    }
